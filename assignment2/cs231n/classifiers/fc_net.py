...
#         pass
    ...
    def loss(self, X, y=None):
        ...
        input - fc1 - hidder - fc2 - output
        fc1 = X * W1 +b1 (N, D) * (D, H) = (N, H)
        hidder = maximum(0, fc1) 
        fc2 = hidder * W2 + b2 (N, H) * (H, C) = (N, C)
        output = softmax(fc2)
        """
        scores = None 
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    [batch norm] - relu - [dropout] 并不影响输出dim
...
#                 beta: Shift paremeter of shape (H,)
...
#         pass
        ...
        for l in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                ...
                layer_input, ar_cache[l] = affine_bn_relu_forward(layer_input, W, b, gamma, beta, self.bn_params[l])
            else:
                layer_input, ar_cache[l] = affine_relu_forward(layer_input, W, b) 

            if self.use_dropout:
                layer_input, dp_cache[l] = dropout_forward(layer_input, self.dropout_param)
...
#         pass
        ...
        layer.reverse()
        for lr in layer:
            loss += 0.5 * self.reg * np.sum(self.params['W%d' % (lr+1)] * self.params['W%d' % (lr+1)])
            if self.use_dropout:
                dout = dropout_backward(dout, dp_cache[lr])
            if self.use_batchnorm:
                dout, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, ar_cache[lr])
                grads['gamma%d' % (lr+1)] = dgamma
                grads['beta%d' % (lr+1)] = dbeta
            else:
                dout, dw, db = affine_relu_backward(dout, ar_cache[lr])
            grads['W%d' % (lr+1)] = dw + self.reg * self.params['W%d'%(lr+1)]
            grads['b%d' % (lr+1)] = db
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
